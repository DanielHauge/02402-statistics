...
Symbol hax: https://rpruim.github.io/s341/S19/from-class/MathinRmd.html


# Beskrivende analyse

## a) Data
Datamaterialet er 145 observationer med relevante egenskaber på mennesker for projektets problemstilling. 
Variable på hver observation er som følger:

| Variable Navn | Måle type | Måle enhed | Forklaring |
| --- | --- | --- | --- |
| Køn | Kategoriserende | 0 (Mand) / 1 (Kvinde) | Personens køn angivet som som et tal ved 0 eller 1.
| Højde | Kvantitativt | cm | Personens højde i centimeter.
| Vægt | Kvantitativt | kg | Personens vægt målt i kilogram
| Urbanitet | Kategoriserende | tal i intervallet [1 - 5] | Befolkningstætheden i byen personen bor. 1: Udenfor bymæssig bebyggelse, 2: Under 10.000 indbyggere, 3: [10.00-49.999] Indbyggere, 4: [50.000-99.999] Indbyggere, 5: Over 100.000 indbyggere
...
```
Det samme mønster opstår, med en højre skæv fordeling. Dog lader det til at stikprøven for mænd er fordelt en lille smule mere skæv set ved en højere difference mellem middelværdien og medianen. Generelt ligger mænd desuden også lidt højere på BMI end kvinder.
...
Med en tabel udfyldt med nøgletal, kan vi mere præcist beskrive fordelingerne. Med box-plottet kunne man med gode øjne spotte sig til at stikprøven for mænd har en mindre spredning, men disse nøgletal afslører det direkte. Med histogrammerne og box-plottet er det således at man ikke har præcise værdier som eksempelvis nedre og øvre kvatiler. Plots afslører ofte heller ikke en præcis tæthed eller data centralitet, men giver blot en illustrerende ide om fordelingen. Disse nøgletal giver et indblik i præcis hvor data'ens centrale tendens er, og hvilke størelser stikprøvernes fordelingen karakteriseres ved. 
...
Hvor $\tilde{x}$ er middelværdien, $s$ er standard afvigelsen og $t_{0.975}$ er t fordelingens 97.5 kvantile i $n-1$ frihedsgrader, hvor n er antal observationer af data'en og derfor 144 frihedsgrader.

```{r}
konf <- mean(D$logbmi)+(qt(0.975, (n-1))*(sd(D$logbmi)/sqrt(length(D)))*c(-1,1))
```
...
### Mænd:
- $M_i \sim LN(\alpha_k, \beta_k^{2} ) (\beta_k>0), i=1,...n_M$
...
```
Estimationer baseret på stikprøven:
- $\alpha_k\approx 3.26058769$
- $\beta_k \approx 0.12391144$
...
- Logbmi middelværdi i stikprøven for kvinder: $\tilde{x_1}=3.17409$
- Standardafvigelse for mænd : $s_1 \approx 0.123$
- Standardafvigelse kvinder : $s_1 \approx 0.159$
- Antal observationer for kvinder: 73
- Antal observationer for kvinder: 72
...
Hvis vi er 95% sikre på at middelværdierne i 2 forskellige uafnhægige populationer ligger i 2 intervaller som ingen fællesværdier har, er vi også 95% sikre på at de 2 populationers middelværdi er forskellig.

Derfor kunne vi blot ved at kigge på konfidensintervalerne, afvise hypotesen om at kvinder og mænd har samme logbmi middelværdi.
...
```
Korrelations kooficienterne er som forventet. Korrelationen mellem vægt og bmi er omkring 0.8 hvilket i min optik viser korrelation, dermed ser man også på plottet at de følges, når bmi bliver høj bliver vægt også højere. Derudover ser vi også at korrelationerne for de konfigurationer der indeholder fastfood ikke er særlig høje, hvilket også kan ses på de meget flade "linjer" data'en danner. Dog kan dette skyldes data representationen. Måske ville det se anderledes ud hvis der var et præcist antal fastfood besøg.
